...
- Console OpenCode (session de l'opérateur, vers 15:35 UTC) : « Use balance » était ACTIVÉ, désactivé ; usage hebdomadaire Go 100,5 % (GLM 5.3 : 7,54 $ sur 7,50 $), mensuel 50,3 % ; « modèles à entraînement » désactivé (constat) ; « modèles hébergés en Chine » ACTIVÉ, conservé par décision écrite de l'opérateur (résidence déclarée : États-Unis ou Chine selon le modèle) ; deux clés (`quantum` = agents, `Default API Key` = poste) ; solde 52,87 $, aucune limite mensuelle de compte, recharge automatique désactivée. VERIFIED.
