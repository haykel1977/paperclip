...
- `hermes_local`: runs your local Hermes CLI (`@henkey/hermes-paperclip-adapter` or a local `file:` path)
...
1. Check adapter command availability (e.g. `claude`/`codex`/`opencode`, or external plugin CLIs such as `hermes`, installed and logged in).
...
1. Choose adapter (e.g. `claude_local`, `codex_local`, `opencode_local`, `cursor`, or `openclaw_gateway`). External plugins like `hermes_local` and `droid_local` are available via the adapter manager.
