...
| Bloc provider `openrouter` | absent du plan | Déclaré dans l'environnement (modèles `anthropic/claude-*`, `openai/gpt-5*`, `google/gemini-2.5-pro`, `x-ai/grok-4-fast`). Le rapport tiers affirmait « 0 appel » : REFUTED par la table `cost_events` le 2026-09-03 : 97 appels OpenRouter entre 07:58 et 13:25 UTC le 2026-09-03 par les quatre agents, dont 54 vers `anthropic/claude-sonnet-4.5` (15,35 $), 41 vers `moonshotai/kimi-k2-0905` (1,16 $) et 2 vers `moonshotai/kimi-k2.7-code` (1,44 $). Du code et des tickets Quantum ont donc transité par Anthropic via OpenRouter pendant environ 5 h 30 ce jour-là, avant la bascule OpenCode Go (13:38 UTC). Clé retirée et bloc supprimé le 2026-09-03. | VERIFIED (SQL `cost_events`) ; « 0 appel » REFUTED |
...
Historique des fournisseurs ayant reçu des données des quatre agents (table `cost_events`, VERIFIED le 2026-09-03) : passerelle Bifrost souveraine du 2026-09-02 09:50 au 2026-09-03 10:38 UTC (dernier appel, Q-Web) ; OpenRouter le 2026-09-03 de 07:58 à 13:25 UTC (Anthropic `claude-sonnet-4.5`, Moonshot `kimi-k2-0905` et `kimi-k2.7-code`, 97 appels, 17,95 $) ; OpenCode Go depuis le 2026-09-03 13:38 UTC. Anthropic figure dans la liste des fournisseurs nommément interdits par le dépôt quantum : cet egress du 2026-09-03 matin est un fait accompli antérieur à cette ADR, déclaré ici, non couvert par elle.
...
Dépense comptabilisée par Paperclip depuis le 2026-09-01 (table `cost_events`, mois UTC) : Q-Gov 589 cents, Q-Impl 946 cents, Q-Web 236 cents, QA-Tests 25 cents ; ces montants proviennent à plus de 99 % des appels OpenRouter du 2026-09-03 matin (historique ci-dessus). Les appels OpenCode Go ne sont pratiquement pas capturés par Paperclip (1 événement de 1 cent contre des dizaines de requêtes visibles dans la console OpenCode le même jour) : la comptabilité Paperclip est aveugle au fournisseur live. VERIFIED (SQL ; console). La console OpenCode fait foi : usage hebdomadaire Go à 100,5 % le 2026-09-03 (GLM 5.3 : 7,54 $ sur un quota hebdomadaire de 7,50 $), usage mensuel à 50,3 % après trois jours.
...
- Portée réelle des budgets Paperclip (constat du 2026-09-03) : la comptabilité Paperclip ne capture pas les appels OpenCode Go (voir D2) ; les budgets et le hard stop posés le 2026-09-03 ne protègent donc que contre un retour d'usage OpenRouter ou Bifrost, pas contre l'usage Go. Les seuls plafonds effectifs sur l'usage Go sont ceux du plan (60 $ mois, 30 $ semaine, 12 $ par 5 h, plafonds par modèle) avec « Use balance » désactivé (fait le 2026-09-03 dans la console : l'option était ACTIVÉE, le dépassement hebdomadaire était donc financé par le solde Zen). Incident du 2026-09-03 15:27 UTC : la politique de Q-Impl (9 $) a déclenché un hard stop immédiat, le dépensé du mois (9,46 $) étant déjà supérieur ; résolu par relèvement à 12 $ et reprise (somme flotte 37 $). QA-Tests mis en pause manuelle à 15:39 UTC en application de D10.6 (quota hebdomadaire GLM 5.3 atteint), jusqu'à la réinitialisation hebdomadaire ou une décision de l'opérateur.
...
- Règle (PROPOSED) : la clé OpenCode Go du poste de travail (workspace IDE) et celle des agents (workspace agents) restent distinctes ; la clé IDE ne va pas sur le serveur. État live de cette séparation : UNVERIFIABLE depuis le fact pack (une seule variable de clé côté conteneur), à confirmer dans la console OpenCode à la signature. Constat console du 2026-09-03 : deux clés API dans le workspace, `quantum` (préfixe identique à la clé du conteneur : clé des agents) et `Default API Key` (poste) : la séparation des clés existe (VERIFIED) ; un seul workspace visible, pas deux comme le plan le voulait. La régénération de la clé `quantum`, qui a transité en clair, reste un acte de l'opérateur (création dans la console, remplacement dans le fichier d'environnement de l'hôte, recréation du conteneur, suppression de l'ancienne).
...
| Console OpenCode (Use balance, plafonds atteints, réglages fournisseurs, clés, solde) | VERIFIED le 2026-09-03 dans la session de l'opérateur | console OpenCode |
| Appels OpenRouter (Anthropic, Moonshot) des quatre agents le 2026-09-03 de 07:58 à 13:25 UTC | VERIFIED | SQL `cost_events` |
| Comptabilité Paperclip aveugle aux appels OpenCode Go | VERIFIED (1 événement en base contre des dizaines dans la console) | SQL `cost_events` ; console |
