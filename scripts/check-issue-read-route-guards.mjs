...
const issueReadGuardPatterns = [
  /\bassertIssueReadAllowed\s*\(\s*req\s*,\s*res\s*,/,
  /\bassertIssueCostReadAllowed\s*\(\s*req\s*,\s*res\s*,/,
  /action\s*:\s*["']issue:read["']/,
];
...
const issueScopedBlockPatterns = [
  /\bassertCompanyAccess\s*\(\s*req\s*,\s*issue\.companyId\s*\)/,
  /\bgetAttachmentById\s*\(/,
  /\bworkProductsSvc\.getById\s*\(/,
];
const boardOnlyPatterns = [
  /\bassertBoard\s*\(\s*req\s*\)/,
...
export function findIssueReadRoutesMissingBoundaryGuardInText(text, filePath = "<input>") {
  const guardedHelpers = collectIssueReadGuardHelpers(text);
  ...
  for (let index = 0; index < routeMatches.length; index += 1) {
    ...
    if (!routeUsesIssueReadScope(method, route, routeBlock)) {
      continue;
    }

    if (isBoardOnlyRoute(routeBlock)) {
      continue;
    }

    if (hasIssueReadGuard(routeBlock) || routeCallsGuardedHelper(routeBlock, guardedHelpers)) {
      continue;
    }
...
export function findIssueReadRoutesMissingBoundaryGuard({ root = process.cwd(), routesDir = defaultRoutesDir } = {}) {
  const offenses = [];
  for (const file of listRouteFiles(root, routesDir)) {
    const relativePath = relative(root, file);
    offenses.push(...findIssueReadRoutesMissingBoundaryGuardInText(readFileSync(file, "utf8"), relativePath));
  }
  return offenses;
}

export function runIssueReadRouteGuardCheck({
  root = process.cwd(),
  routesDir = defaultRoutesDir,
  log = console.log,
  error = console.error,
} = {}) {
  const offenses = findIssueReadRoutesMissingBoundaryGuard({ root, routesDir });

  if (offenses.length > 0) {
    for (const offense of offenses) {
      error(
        `${offense.filePath}:${offense.lineNumber}: ${offense.method} ${offense.route} must validate issue read authorization with assertIssueReadAllowed/assertIssueCostReadAllowed or an equivalent guarded helper.`,
      );
    }
    error("Issue-scoped read routes must enforce issue:read boundaries, not just company membership.");
    return 1;
  }

  log("  ✓  Issue-scoped read routes validate issue read boundaries.");
  return 0;
}
...
if (isMainModule) {
  process.exit(runIssueReadRouteGuardCheck());
}
