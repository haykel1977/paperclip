...
function buildWorkspaceCommandEnv(input: {
  ...
  if (input.issue?.description != null && input.issue.description !== "") {
    env.PAPERCLIP_ISSUE_DESCRIPTION = input.issue.description;
  }
  env.PAPERCLIP_ISSUE_WORK_MODE = input.issue?.workMode ?? "";
  const existingGithubIssueNumber = env.PAPERCLIP_GITHUB_ISSUE_NUMBER?.trim() ?? "";
  if (!existingGithubIssueNumber) {
    const recovered =
