...
describe("realizeExecutionWorkspace", () => {
  ...
  it("starts a git worktree from the refreshed upstream when baseRef names a local branch", async () => {
    const sourceRepo = await createTempRepo("master");
    const remoteDir = await fs.mkdtemp(path.join(os.tmpdir(), "paperclip-worktree-remote-"));
    const remotePath = path.join(remoteDir, "paperclip.git");
    await execFileAsync("git", ["clone", "--bare", sourceRepo, remotePath]);
    ...
    const staleLocalHead = await readGit(repoRoot, ["rev-parse", "master"]);
    ...
    expect(await readGit(workspace.cwd, ["rev-parse", "HEAD"])).toBe(expectedRemoteHead);
    expect(await readGit(repoRoot, ["rev-parse", "origin/master"])).toBe(expectedRemoteHead);
    expect(workspace.warnings.some((w) => w.includes("upstream origin/master"))).toBe(true);
    // The stale local branch itself is left untouched (never rewritten behind the operator's back).
    expect(await readGit(repoRoot, ["rev-parse", "master"])).toBe(staleLocalHead);
  });
