...
describe("codex execute", () => {
  ...
  it.each([
    ...
    async ({ model, baseUrlEnvKey }) => {
      ...
      process.env[baseUrlEnvKey] = "https://litellm.kantum.dev/v1";
      ...
      try {
        ...
        if (previousOpenAiApiBase === undefined) delete process.env.OPENAI_API_BASE;
        else process.env.OPENAI_API_BASE = previousOpenAiApiBase;
        if (previousOpenAiApiBaseUrl === undefined) delete process.env.OPENAI_API_BASE_URL;
        else process.env.OPENAI_API_BASE_URL = previousOpenAiApiBaseUrl;
