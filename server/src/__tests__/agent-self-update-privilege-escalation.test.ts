...
function resetMockDefaults() {
  ...
  mockAccessService.decide.mockImplementation(async (input: { actor?: { type?: string; source?: string }; action?: string }) => {
    const allowed = input.actor?.type === "agent"
      ? true
...
describe.sequential("agent self-update privilege escalation", () => {
  beforeEach(() => {
    resetMockDefaults();
  });
  ...
  const escalationCases = [
    { label: "role", payload: { role: "ceo" } },
    { label: "status", payload: { status: "active" } },
    { label: "reportsTo", payload: { reportsTo: null } },
    { label: "budgetMonthlyCents", payload: { budgetMonthlyCents: 999999 } },
    { label: "spentMonthlyCents", payload: { spentMonthlyCents: 0 } },
    { label: "adapterType", payload: { adapterType: "process" } },
    { label: "defaultEnvironmentId", payload: { defaultEnvironmentId: null } },
  ];

  it.each(escalationCases)(
    "refuses an agent-authenticated self-update of $label",
    async ({ payload }) => {
      const app = await createApp(selfActor);
      const response = await requestApp(app, (baseUrl) =>
        request(baseUrl).patch(`/api/agents/${agentId}`).send(payload));
  ...
  it("still allows an agent to update its own non-privileged fields", async () => {
    const app = await createApp(selfActor);
    const response = await requestApp(app, (baseUrl) =>
      request(baseUrl).patch(`/api/agents/${agentId}`).send({ title: "Builder of things" }));

    expect(response.status).toBe(200);
    expect(mockAgentService.update).toHaveBeenCalled();
  });

  it("still allows a board caller to change an agent role", async () => {
